...
public record LegendaryAssignments(Map<Form, Identifier> byForm) {
	// JSON codec (persisted)
	public static final Codec<LegendaryAssignments> CODEC = RecordCodecBuilder.create(i -> i.group(
			Codec.unboundedMap(Form.CODEC, Identifier.CODEC).fieldOf("by_form").forGetter(LegendaryAssignments::byForm)
	...
	public static final StreamCodec<ByteBuf, LegendaryAssignments> PACKET_CODEC = StreamCodec.composite(
			ByteBufCodecs.map(HashMap::new, Form.STREAM_CODEC, Identifier.STREAM_CODEC), LegendaryAssignments::byForm,
	...
	public Set<Form> extrasFor(Identifier materialId) {
	...
	public static LegendaryAssignments assign(Random rng, List<Identifier> materials, List<Form> uniqueForms) {
		Map<Form, Identifier> map = new EnumMap<>(Form.class);
		...
		List<Identifier> mats = new ArrayList<>(materials);
